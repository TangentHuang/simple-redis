...
use crate::cmd::{
    extract_args, validate_command, CmdHGet, CmdHGetAll, CmdHSet, CommandError, CommandExecutor,
    RESP_OK,
};
use crate::resp::{BulkString, RespArray, RespFrame, RespNull};

impl CommandExecutor for CmdHGet {
...
impl CommandExecutor for CmdHSet {
    fn execute(self, backend: &Backend) -> RespFrame {
...
impl CommandExecutor for CmdHGetAll {
    fn execute(self, backend: &Backend) -> RespFrame {
...
impl TryFrom<RespArray> for CmdHGet {
    type Error = CommandError;
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        validate_command(&value, &["hget"], 2)?;
        let mut args = extract_args(value, 1)?.into_iter();
        match (args.next(), args.next()) {
            (Some(RespFrame::BulkString(key)), Some(RespFrame::BulkString(field))) => Ok(CmdHGet {
...
impl TryFrom<RespArray> for CmdHSet {
    type Error = CommandError;
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
...
impl TryFrom<RespArray> for CmdHGetAll {
    type Error = CommandError;
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        validate_command(&value, &["hgetall"], 1)?;
        let mut args = extract_args(value, 1)?.into_iter();
        match args.next() {
            Some(RespFrame::BulkString(key)) => Ok(CmdHGetAll {
                key: String::from_utf8(key.0)?,
                sort: false,
            }),
...
mod test {
    ...
    fn test_hget_from_resp_array() -> Result<()> {
        ...
        let result: CmdHGet = frame.try_into()?;
    ...
    fn test_hgetall_from_resp_array() -> Result<()> {
        ...
        let result: CmdHGetAll = frame.try_into()?;
        assert_eq!(result.key, "map");
    ...
    fn test_hset_from_resp_array() -> Result<()> {
        ...
        let result: CmdHSet = frame.try_into()?;
        assert_eq!(result.key, "map");
        assert_eq!(result.field, "hello");
